...
const Products = () => {
    ...
    const priceRangeFilter = (showProduct, filterOnPriceRange) => {
        if (filterOnPriceRange !== null) {
          return showProduct.filter(
            (item) => item.price <= filterOnPriceRange
          );
        } else {
          return showProduct;
        }
    ...
    const ratingFilter = (showProduct, filterOnRating) => {
                ...
                {filteredItems(filterState).length > 0 && 
                 filteredItems(filterState).map((item => 
                    ...
                    <div class="footer">
                        ...
                        </Link> : <button class="btn primary-btn" onClick={() => cartDispatch({type : "Add_to_cart", payload : item})}>Add to cart</button>
                        }
                        {wishState.wish.find((wishProduct) => wishProduct.id===item.id) ? 
                        <Link to="/wishlist">
                            <button class="btn secondary-btn-border">Go to Wishlist</button>
                        </Link> : <button class="btn secondary-btn-border" onClick={() => wishDispatch({type : "Add_to_wishlist", payload : item})}>Wishlist</button> }
